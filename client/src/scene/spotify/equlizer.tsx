...
class Equalizer extends Component<Props, State> {
  ...
  animate = () => {
    if (this.analyser) {
      ...
      const bars = this.calculateSpectorgram(screen.width, screen.height);
      ...
      this.setState({ bars: bars, rms, beatDetected });
    }
    this.animationFrameId = requestAnimationFrame(this.animate);
  };

  calculateSpectorgram(width: number, height: number) {
    ...
    for (let i = 0; i < numBuckets; i++) {
      ...
      bars.push({ x, y, barWidth, barHeight, color: `hsl(${hue}, 100%, 50%)` });
    }
    return bars;
  }
      ...
      <div>
        ...
        <div
          style={{
            position: "absolute",
            border: "1px solid red",
            height: `${screen.height * this.state.rms * 10}px`,
            right: 0,
            // top: "0",
            bottom: "30%",
            width: "50px",
